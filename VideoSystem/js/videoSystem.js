...
function DefaultCategoryVideoSystemException() {
    this.name = "DefaultCategoryVideoSystemException";
    this.message = "Error: The deafult category can't be removed.";
}
DefaultCategoryVideoSystemException.prototype = new VideoSystemException();
DefaultCategoryVideoSystemException.prototype.constructor = DefaultCategoryVideoSystemException;
            ...
            /*this.getCast = function(production){
                ...
                var nextIndex = 0;
                    return {
                        next: function () {
                            var character = "";
			       			if (_actors[nextIndex].productions[nextIndex].production === production.title){
                                   character = _actors[nextIndex].productions[nextIndex].character;
                                   
                               }
                               nextIndex++;  
                            return nextIndex < _actors.length ?
                            {value: character, done: false} :
                            {done: true};  
                        }
                    }
			}*/
