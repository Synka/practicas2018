        ...
        function VideoSystem() {
            ...
            this.removeProduction = function (production) {
                ...
                if (position !== -1) {
                    ...
                    for (var x = 0; x < cast.actors.length; x++) {
                        var ap = getActorPosition(cast.actors[x].actor);
                        var actp = getProPosition(production, _actors[ap].productions);
                        if (actp !== -1) {
                            _actors[ap].productions.splice(actp, 1);
                        }
                    }

                    var dirp = getProPosition(production, _directors[dp].productions);
                    if (dirp !== -1) {
                        _directors[dp].productions.splice(dirp, 1);
                    }
            ...
            this.addDirector = function (director) {
                ...
                if (position === -1) {
                    ...
                    //addDB(director, "directors", director.name);
                } else {
            ...
            this.getCast = function (production) {
                ...
                var productionPosition = getProductionPosition(production);
                if (productionPosition === -1) throw new ProductionNotExistsVideoSystemException();
                var iterador = {
                    actors: [],
                    director: {}
                };
